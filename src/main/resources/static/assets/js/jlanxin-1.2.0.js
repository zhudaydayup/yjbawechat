...
var LanXinParam = {
	//host : "http://fer01-open.e.lanxin.cn",
	host : "",
	appId : "",
	secret : "",
	signature : "",
...
function thirdLogin(){
	...
	if(isNull(LanXinParam.host)){
		var protocal = localUrl.substring(0, localUrl.indexOf("://"));
		var uri = localUrl.substring(localUrl.indexOf("://") + 3);
		var domain = uri.substring(0, uri.indexOf("/"));
		LanXinParam.host = protocal + "://" + domain;
	}
	var url = LanXinParam.host + "/third/login?appId=" + LanXinParam.appId + "&secret=" + LanXinParam.secret + "&url=" + localUrl + "&isJsonp=1"
	$.ajax({
		type : "get",
		async:false,
		url : url,
		...
		success : function(json){
			//alert("signature : " + json.signature + ", url : " + json.url + ", ticket : " + json.ticket + ", nonce : " + json.nonce + ", timestamp : " + json.timestamp);
			LanXinParam.signature = json.signature;
			LanXinParam.ticket = json.ticket;
			LanXinParam.nonce = json.nonce;
			LanXinParam.timestamp = json.timestamp;
			LanXinParam.url = json.url;

			config({
				debug : false,
				appId : LanXinParam.appId,
				timestamp : LanXinParam.timestamp,
				nonce : LanXinParam.nonce,
				signature : LanXinParam.signature,
				url : LanXinParam.url,
				ticket : LanXinParam.ticket
			});
		},
		error:function(){
			alert('登录失败');
...
//	if(!isNull(LanXinParam.signature)){
//		var url = "http://fer01-open.e.lanxin.cn/third/login?oper=checkApi"+
//	    "&ticket=" + LanXinParam.ticket + 
//		"&signature=" + LanXinParam.signature + 
//		"&nonce=" + LanXinParam.nonce +
//		"&timestamp=" + LanXinParam.timestamp +
...
function config(param) {
	formateJsonAndCallHander("config", param);
}
...
function openNFC(param) {
        formateJsonAndCallHander("openNFC", param);
}
