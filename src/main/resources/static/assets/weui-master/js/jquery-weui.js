...
+ function($) {
  ...
  Select.prototype.open = function(values, titles) {
    ...
    // mqtt picker 会默认关掉其他的，但是 onClose 不会被调用，所以这里先关掉其他select
...
+ function($) {
  ...
  defaults = $.fn.datetimePicker.prototype.defaults = {
    ...
    parse: function (str) {
      // 把字符串转换成数组，用来解析初始值
      // 如果您的定制的初始值格式无法被这个默认函数解析，请自定义这个函数。比如您的时间是 '子时' 那么默认情况这个'时'会被当做分隔符而导致错误，所以你需要自己定义parse函数
      // 默认兼容的分隔符
...
+function ($) {
  ...
  Swipeout.prototype.open = function() {
    ...
    this.container.trigger('swipeout-mqtt');
  }
