        ...
        success: function (data) {
            if (data) {
                ...
                if (data[0].error == "0") {
                    ...
                    alert(signName);
                    alert(RecordId);
                    updateSighRecord(signName,RecordId);
                }
...
//获取url参数
  ...
  //更新签名参数
  function updateSighRecord(signName,RecordId){
      $.ajax({
          type: "POST",
          url:  "../reexamine/updateExpertSign",
          data: {

              CHECKE_SIGNATURE: signName,
              RecordId: RecordId
          },
          dataType: "json",
          async: true,
          success: function (data) {
              if (data.backFlag == "ok") {
                  window.location.href="../page/reexamineInspectedSigh?RecordId="+RecordId+"&social_credit_code="+social_credit_code+"&userId="+userId;
              }
              else {
                  alert("提交失败")
              }
