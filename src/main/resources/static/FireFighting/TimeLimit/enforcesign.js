...
function getUrlParam(key){
  ...
  function updateSighRecord(signName,RecordId){
      $.ajax({
          type: "POST",
          url:  "../timeLimit/UpdateEnforceSigh",
          data: {
          ...
          success: function (data) {
              if (data.backFlag == "ok") {
                  window.location.href="../page/inSpectorSigh?RecordId="+RecordId+"&social_credit_code="+social_credit_code+"&userId="+userId+"&ID="+ID+"&sCode="+sCode;
              }
              else {
                  alert("提交失败")
              }
