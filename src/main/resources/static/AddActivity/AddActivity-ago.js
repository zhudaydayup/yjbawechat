...
function submitRecord() {
    ...
    if( $("#otherbox").css("display") == "none") {
        ...
        ajaxPram["EXPERT_NAME2"] ="";//32专家2
        ajaxPram["EXPERT_UNIT2"] ="";//33专家2所在单位
    }
    else
    {
        //otherexpert
        if ($("#otherexpert").css("display")=="none"){
            ajaxPram["EXPERT_NAME1"] = $("#EXPERT_NAME1").val();//32专家1
            ajaxPram["EXPERT_UNIT1"] = $("#EXPERT_UNIT1").val();//33专家1所在单位
            ajaxPram["EXPERT_NAME2"] ="";//32专家2
            ajaxPram["EXPERT_UNIT2"] ="";//33专家2所在单位
        }
        else{
            ajaxPram["EXPERT_NAME1"] = $("#EXPERT_NAME1").val();//32专家1
            ajaxPram["EXPERT_UNIT1"] = $("#EXPERT_UNIT1").val();//33专家1所在单位
            ajaxPram["EXPERT_NAME2"] = $("#EXPERT_NAME2").val();//32专家1
            ajaxPram["EXPERT_UNIT2"] = $("#EXPERT_UNIT2").val();//33专家1所在单位
        }
    }


    //var isboolture =  ajaxPram["RECORD_ID"]!="" && ajaxPram["CHECKED_START_TIME"] != "" && ajaxPram["CHECKED_END_TIME"] != ""&& ajaxPram["CHECKED_UNIT"] != "" &&  ajaxPram["UNIT_NUMBER"] != "" && ajaxPram["ADDRESS"] != ""&& ajaxPram["REPRESENT_PEOPLE"] != "" &&ajaxPram["REPRESENT_MOBILE"]  != "" && ajaxPram["CHECKED_LOCATION"] != ""&&ajaxPram["CHECKED_REPRESENT_PEOPLE"]!="" && ajaxPram["CHECKED_REPRESENT_DUTY"] != ""&&ajaxPram["CHECKED_REPRESENT_NUMBER"]!=""&& ajaxPram["CHECKED_REPRESENT_MOBILE"] != ""&&ajaxPram["CHECKED_SEX"]!=""&& ajaxPram["CHECK_UNIT1"] != ""&&ajaxPram["CHECK_PEOPLE1"]!=""&& ajaxPram["CARD_NUMBER1"] != "";
    var isboolture =  ajaxPram["RECORD_ID"]!="" && ajaxPram["CHECKED_UNIT"] != ""   && ajaxPram["CHECKED_LOCATION"] != ""&&ajaxPram["CHECKED_REPRESENT_PEOPLE"]!="" && ajaxPram["CHECKED_REPRESENT_DUTY"] != ""&&ajaxPram["CHECKED_REPRESENT_NUMBER"]!=""&& ajaxPram["CHECKED_REPRESENT_MOBILE"] != ""&&ajaxPram["CHECKED_SEX"]!=""&& ajaxPram["CHECK_UNIT1"] != ""&&ajaxPram["CHECK_PEOPLE1"]!=""&& ajaxPram["CARD_NUMBER1"] != "";
    ...
    if (isboolture) {
        $.confirm("确认创建检查记录!", function (){
            ...
            $.post("../Record/UpdateIdRecord", ajaxPram , function (response) {
                ...
                if (odj['backFlag'] == "ok") {
                    alert("创建成功!");
                    location.href = "../LawDictionary/CheckSign?RecordId="+recordId+"";
                } else {
                    alert(odj['failInfo']);
                }
            },"json");
        ...
        success: function (data) {
            if (data.backFlag == "ok") {
                if (data.RecordInfo.length!=0){
                    ...
                    var EXPERT_NAME2= data.RecordInfo[0].eXPERT_NAME2;//专家2
                    var EXPERT_UNIT2=  data.RecordInfo[0].eXPERT_UNIT2;//专家2所在单位
                    if (typeof(EXPERT_NAME1) == "undefined") {
                        /*逻辑*/
                        // $("#othercheck").css("display","none")
                    }
                    else{
                        $("#EXPERT_NAME1").val(EXPERT_NAME1);
                        $("#EXPERT_UNIT1").val(EXPERT_UNIT1);
                    }
                    if (typeof(EXPERT_NAME2) == "undefined"){
                        $("#otherexpert").css("display","none")
                    }
                    else
                    {
                        $("#otherexpert").css("display","block")
                        $("#EXPERT_NAME2").val(EXPERT_NAME2);
                        $("#EXPERT_UNIT2").val(EXPERT_UNIT2);
                    }
