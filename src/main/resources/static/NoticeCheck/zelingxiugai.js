...
function submitLaw() {
    ...
    ajaxPram["NOTICE_DETAIL"] = $("#NOTICE_DETAIL").val();
    ...
    //     }
    // }
    var isboolture =  ajaxPram["CHECKE_UNIT"] != "" && ajaxPram["EXECUTE_PEOPLE"] != "" && ajaxPram["CHECKED_START_TIME"] != "" &&  ajaxPram["CHECKE_PROBLEM_IDS"] != ""&&  ajaxPram["CHECKED_END_TIME"] != ""&&  ajaxPram["NOTICE_DETAIL"] != ""&&  ajaxPram["RECORD_ID"] != "" ;

    if (isboolture) {
            $.confirm("确认创建检查记录!", function (){
            document.getElementById("mainBody").style.display = "none";
            document.getElementById("loadingmore").style.display = "";
            $.post("../NoticeView/createNoticeBiao", ajaxPram , function (response) {
                var odj = response;
                if (odj['backFlag'] == "ok") {
                    alert("创建成功!");
                    var record_id = odj["recordId"];
                    location.href = "../page/CheckedSighView?RecordId="+record_id+"";
                } else {
                    alert(odj['failInfo']);
                }
            },"json");
