(function() {
    ...
    var Pdfh5 = function(dom, options) {
        this.container = $(dom);
        this.currentNum = 1; //褰撳墠椤垫暟浠�1寮€濮�
        this.thePDF = null;
        this.pdfRender = null;
        this.totalNum = null;
        ...
        init: function(options) {
            ...
            if (pdfurl) {
                ...
                setTimeout(function() {
                    ...
                    self.eventType["complete"] && self.eventType["complete"].call(self, "error", "鏂囦欢璺緞涓嶈兘涓虹┖", self.endTime);
                    self.eventType["error"] && self.eventType["complete"].call(self, "error", "鏂囦欢璺緞涓嶈兘涓虹┖", self.endTime);
                    self.complete && self.complete("error", "鏂囦欢璺緞涓嶈兘涓虹┖", self.endTime)
                    self.error && self.error("error", "鏂囦欢璺緞涓嶈兘涓虹┖", self.endTime)
                }, 0)
            }
            ...
            function handlePages(page) {
                ...
                self.pdfRender.promise.then(function() {
                    ...
                    if (!self.pdfLoaded && self.thePDF && self.currentNum <= self.totalNum) {
                        ...
                        self.eventType["complete"] && self.eventType["complete"].call(self, "success", "PDF瑙ｆ瀽瀹屾瘯", self.endTime);
                        self.eventType["success"] && self.eventType["success"].call(self, "success", "PDF瑙ｆ瀽瀹屾瘯", self.endTime);
                        self.complete && self.complete("success", "PDF瑙ｆ瀽瀹屾瘯", self.endTime)
                        self.success && self.success("success", "PDF瑙ｆ瀽瀹屾瘯", self.endTime)
