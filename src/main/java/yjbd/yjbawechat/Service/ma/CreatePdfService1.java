...
@Service
public class CreatePdfService1 {
    ...
    static final Logger logger = LoggerFactory.getLogger(CreatePdfService1.class);
    /**
     * 生成整改意见复查文书表格
     * @param RECORD_ID
     * @return
     * @throws IOException
     */
    public String createPDF1(String RECORD_ID) throws IOException {
        Document document = new Document(PageSize.A4);
        String pdfPath="D:/zfbdzl/pdf/fucha/";
        String qrCodePath="D:/zfbdzl/qrcode/fucha/";
        String url="D:/zfbdzl/images/";
        String pdfAbsolutePath="http://zfxc.njyjgl.cn/yjbd2/pdf/getPdf?url=fucha/";
        ...
        //设置日期格式
        List<Map> List = rectificationDao.getPdfInfo(RECORD_ID);

        List<Map<String,Object>> list=pdfThreeDal.getThreeById(RECORD_ID);
//        String ZELINGUPID="";//责令整改自增id
//        if(list.get(0).get("UPID")!=null) ZELINGUPID=list.get(0).get("UPID").toString();

        //从表格中拉取证件号信息
        List<Map>mapList=rectificationDao.searchCareId(RECORD_ID);
        ...
        String SIGNATURE6_PATH="";
        String SIGNATURE7_PATH="";
        String SIGNATURE8_PATH="";
        String SIGNATURE9_PATH="";
        String RECORD_TIME="";
//        String AREA="宁";
...
//        if(mapList.get(0).get("CARD_NUMBER2")!=null)  CARD_NUMBER2 = mapList.get(0).get("CARD_NUMBER2").toString ();
        ...
        if(List.get(0).get("SIGNATURE4_PATH")!=null)  SIGNATURE4_PATH = List.get(0).get("SIGNATURE4_PATH").toString ();
        if(List.get(0).get("SIGNATURE5_PATH")!=null)  SIGNATURE5_PATH = List.get(0).get("SIGNATURE5_PATH").toString ();
        if(List.get(0).get("SIGNATURE6_PATH")!=null)  SIGNATURE6_PATH = List.get(0).get("SIGNATURE6_PATH").toString ();
        if(List.get(0).get("SIGNATURE7_PATH")!=null)  SIGNATURE7_PATH = List.get(0).get("SIGNATURE7_PATH").toString ();
        if(List.get(0).get("SIGNATURE8_PATH")!=null)  SIGNATURE8_PATH = List.get(0).get("SIGNATURE8_PATH").toString ();
        if(List.get(0).get("SIGNATURE9_PATH")!=null)  SIGNATURE9_PATH = List.get(0).get("SIGNATURE9_PATH").toString ();
        ...
        String[] SIGNATURE6_PATH_ARRAY= SIGNATURE6_PATH.split("\\|");
        String[] SIGNATURE7_PATH_ARRAY= SIGNATURE7_PATH.split("\\|");
        String[] SIGNATURE8_PATH_ARRAY= SIGNATURE8_PATH.split("\\|");
        String[] SIGNATURE9_PATH_ARRAY= SIGNATURE9_PATH.split("\\|");
        try {
...
//            document.add(new Chunk(lineSeparator));
            ...
            //时间
            Chunk chunk1 = new Chunk("         本机关于"+CHECKED_START_TIME+"作出了责令限期整改书[("+AREA+" )应急字责改[2019] ("+UPID+")号]，经对你单位整改情况进行复查，提出如下意见:",para);
            document.add(chunk1);
            document.add(Chunk.NEWLINE);
...
//            }
          ...
          /*  PdfContentByte yz= writer.getDirectContent();
            ...
            rectificationDao.updatePdfAndQrcode(pdfName,qrCodeName,RECORD_ID);
